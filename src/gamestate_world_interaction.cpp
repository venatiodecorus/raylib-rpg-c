#include "ecs_gameplay_components.h"
#include "gamestate.h"

/** @file gamestate_world_interaction.cpp
 *  @brief Movement, interaction, pickup, stairs, and world-physics helpers on `gamestate`.
 */

static string interaction_state_sentence(const char* noun, bool is_open) {
    return TextFormat("The %s stands %s.", noun, is_open ? "open" : "closed");
}

static string interaction_chest_text(const string& base_text, bool is_open) {
    return TextFormat("%s %s", base_text.c_str(), interaction_state_sentence("lid", is_open).c_str());
}

static string interaction_door_text(const string& base_text, bool is_open) {
    return TextFormat("%s %s", base_text.c_str(), interaction_state_sentence("door", is_open).c_str());
}

entityid gamestate::tile_has_pullable(int x, int y, int z) {
    minfo("tile_has_pullable(%d, %d, %d)", x, y, z);
    massert(z >= 0, "floor is out of bounds");
    massert(static_cast<size_t>(z) < d.floors.size(), "floor is out of bounds");
    shared_ptr<dungeon_floor> df = d.get_floor(z);
    tile_t& t = df->tile_at(vec3{x, y, z});
    entityid box_id = t.get_cached_box();
    if (box_id != INVALID) {
        bool is_pullable = get_component_or<PullableTag>(box_id, false);
        if (is_pullable) {
            return box_id;
        }
    }
    entityid chest_id = t.get_cached_chest();
    if (chest_id != INVALID) {
        bool is_pullable = get_component_or<PullableTag>(chest_id, false);
        if (is_pullable) {
            return chest_id;
        }
    }
    entityid dead_npc_id = t.get_cached_dead_npc();
    if (dead_npc_id != INVALID) {
        bool is_pullable = get_component_or<PullableTag>(dead_npc_id, false);
        if (is_pullable) {
            return dead_npc_id;
        }
    }
    entityid prop_id = t.get_cached_prop();
    if (prop_id != INVALID) {
        bool is_pullable = get_component_or<PullableTag>(prop_id, false);
        if (is_pullable) {
            return prop_id;
        }
    }
    return ENTITYID_INVALID;
}

bool gamestate::tile_has_solid(int x, int y, int z) {
    massert(z >= 0, "floor is out of bounds");
    massert(static_cast<size_t>(z) < d.floors.size(), "floor is out of bounds");
    shared_ptr<dungeon_floor> df = d.get_floor(z);
    tile_t& t = df->tile_at(vec3{x, y, z});

    entityid id = t.get_cached_live_npc();
    bool is_solid = get_component_or<SolidTag>(id, false);
    if (id != ENTITYID_INVALID && is_solid) {
        return true;
    }

    id = t.get_cached_box();
    is_solid = get_component_or<SolidTag>(id, false);
    if (id != ENTITYID_INVALID && is_solid) {
        return true;
    }

    id = t.get_cached_chest();
    is_solid = get_component_or<SolidTag>(id, false);
    if (id != ENTITYID_INVALID && is_solid) {
        return true;
    }

    id = t.get_cached_prop();
    is_solid = get_component_or<SolidTag>(id, false);
    if (id != ENTITYID_INVALID && is_solid) {
        return true;
    }

    id = t.get_cached_door();
    is_solid = get_component_or<SolidTag>(id, false);
    if (id != ENTITYID_INVALID && is_solid) {
        return true;
    }

    id = t.get_cached_item();
    is_solid = get_component_or<SolidTag>(id, false);
    if (id != ENTITYID_INVALID && is_solid) {
        return true;
    }

    return false;
}

bool gamestate::handle_box_push(entityid id, vec3 v) {
    bool can_push = get_component_or<PushableTag>(id, false);
    if (!can_push) {
        return false;
    }
    if (processing_actions) {
        return queue_push_event(id, v);
    }
    return try_entity_push(id, v);
}

bool gamestate::try_entity_push(entityid id, vec3 v) {
    return try_entity_move(id, v);
}

entityid gamestate::tile_has_pushable(int x, int y, int z) {
    massert(z >= 0, "floor is out of bounds");
    massert(static_cast<size_t>(z) < d.floors.size(), "floor is out of bounds");
    shared_ptr<dungeon_floor> df = d.get_floor(z);
    tile_t& t = df->tile_at(vec3{x, y, z});

    entityid id = t.get_cached_box();
    bool is_pushable = get_component_or<PushableTag>(id, false);
    if (id != ENTITYID_INVALID && is_pushable) {
        return id;
    }

    id = t.get_cached_chest();
    is_pushable = get_component_or<PushableTag>(id, false);
    if (id != ENTITYID_INVALID && is_pushable) {
        return id;
    }

    id = t.get_cached_prop();
    is_pushable = get_component_or<PushableTag>(id, false);
    if (id != ENTITYID_INVALID && is_pushable) {
        return id;
    }

    return ENTITYID_INVALID;
}

entityid gamestate::tile_has_door(vec3 v) {
    massert(v.z >= 0, "floor is out of bounds");
    massert(static_cast<size_t>(v.z) < d.floors.size(), "floor is out of bounds");
    shared_ptr<dungeon_floor> df = d.get_floor(static_cast<size_t>(v.z));
    tile_t& t = df->tile_at(v);
    return t.get_cached_door();
}

bool gamestate::check_hearing(entityid id, vec3 loc) {
    if (id == ENTITYID_INVALID || vec3_invalid(loc)) {
        return false;
    }
    vec3 hero_loc = get_component_or<Position>(hero_id, vec3{-1, -1, -1});
    if (vec3_invalid(hero_loc) || hero_loc.z != loc.z) {
        return false;
    }

    float x0 = hero_loc.x;
    float y0 = hero_loc.y;
    float x1 = loc.x;
    float y1 = loc.y;
    Vector2 p0 = {x0, y0};
    Vector2 p1 = {x1, y1};

    float dist = Vector2Distance(p0, p1);
    float hearing = get_component_or<HearingRange>(hero_id, 3);
    return dist <= hearing;
}

bool gamestate::tile_has_pressure_plate_occupant(vec3 loc) {
    if (vec3_invalid(loc) || loc.z < 0 || static_cast<size_t>(loc.z) >= d.floors.size()) {
        return false;
    }

    tile_t& tile = d.get_floor(static_cast<size_t>(loc.z))->tile_at(loc);
    if (tile.get_cached_player_present()) {
        return true;
    }
    if (tile.get_cached_live_npc() != ENTITYID_INVALID || tile.get_dead_npc_count() > 0) {
        return true;
    }

    const entityid box_id = tile.get_cached_box();
    if (box_id != ENTITYID_INVALID && (get_component_or<PushableTag>(box_id, false) || get_component_or<PullableTag>(box_id, false))) {
        return true;
    }

    const entityid chest_id = tile.get_cached_chest();
    if (chest_id != ENTITYID_INVALID && (get_component_or<PushableTag>(chest_id, false) || get_component_or<PullableTag>(chest_id, false))) {
        return true;
    }

    const entityid prop_id = tile.get_cached_prop();
    if (prop_id != ENTITYID_INVALID && (get_component_or<PushableTag>(prop_id, false) || get_component_or<PullableTag>(prop_id, false))) {
        return true;
    }

    return false;
}

bool gamestate::door_is_pressure_plate_controlled(entityid door_id) const {
    if (door_id == ENTITYID_INVALID) {
        return false;
    }
    for (const floor_pressure_plate_t& plate : floor_pressure_plates) {
        if (!plate.destroyed && plate.linked_door_id == door_id) {
            return true;
        }
    }
    return false;
}

floor_pressure_plate_t* gamestate::get_floor_pressure_plate(vec3 loc) {
    for (floor_pressure_plate_t& plate : floor_pressure_plates) {
        if (vec3_equal(plate.loc, loc)) {
            return &plate;
        }
    }
    return nullptr;
}

void gamestate::update_pressure_plates_for_floor(int z) {
    vector<entityid> linked_doors;
    for (floor_pressure_plate_t& plate : floor_pressure_plates) {
        if (plate.loc.z != z || plate.destroyed) {
            continue;
        }

        const bool active = tile_has_pressure_plate_occupant(plate.loc);
        if (plate.linked_door_id == ENTITYID_INVALID ||
            (get_component<EntityTypeTag>(plate.linked_door_id) ? get_component<EntityTypeTag>(plate.linked_door_id)->type : entitytype_t::NONE) !=
                entitytype_t::DOOR) {
            plate.linked_door_id = ENTITYID_INVALID;
            plate.active = false;
            continue;
        }

        if (std::find(linked_doors.begin(), linked_doors.end(), plate.linked_door_id) == linked_doors.end()) {
            linked_doors.push_back(plate.linked_door_id);
        }
        plate.active = active;
    }

    for (const entityid door_id : linked_doors) {
        bool should_open = false;
        for (const floor_pressure_plate_t& plate : floor_pressure_plates) {
            if (plate.destroyed || plate.linked_door_id != door_id) {
                continue;
            }
            if (plate.active) {
                should_open = true;
                break;
            }
        }
        if (processing_actions) {
            queue_pressure_plate_set_door_event(door_id, should_open);
        }
        else {
            resolve_pressure_plate_set_door_event(door_id, should_open);
        }
    }

    frame_dirty = true;
}

void gamestate::resolve_pressure_plate_set_door_event(entityid door_id, bool should_open) {
    if (door_id == ENTITYID_INVALID ||
        (get_component<EntityTypeTag>(door_id) ? get_component<EntityTypeTag>(door_id)->type : entitytype_t::NONE) != entitytype_t::DOOR) {
        return;
    }

    const bool was_open = get_component_or<DoorOpenFlag>(door_id, false);
    sync_registry_open_state(door_id, should_open);
    if (!was_open && should_open) {
        audio.queue("sfx/Minifantasy_Dungeon_SFX/01_chest_open_1.wav");
    }
}

void gamestate::refresh_pressure_plates() {
    for (size_t z = 0; z < d.floors.size(); z++) {
        update_pressure_plates_for_floor(static_cast<int>(z));
    }
}

void gamestate::clear_gameplay_events() {
    gameplay_events.clear();
}

bool gamestate::queue_gameplay_event(const gameplay_event_t& event) {
    if (event.type == event_type_t::NONE) {
        return false;
    }
    gameplay_events.push_back(event);
    return true;
}

bool gamestate::queue_move_event(entityid id, vec3 v) {
    gameplay_event_t event;
    event.type = event_type_t::MOVE_INTENT;
    event.actor_id = id;
    event.delta = v;
    return queue_gameplay_event(event);
}

bool gamestate::queue_push_event(entityid id, vec3 v) {
    gameplay_event_t event;
    event.type = event_type_t::PUSH_INTENT;
    event.actor_id = id;
    event.delta = v;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_event(entityid id, vec3 loc) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_INTENT;
    event.actor_id = id;
    event.target_loc = loc;
    return queue_gameplay_event(event);
}

bool gamestate::queue_provoke_npc_event(entityid npc_id, entityid source_id) {
    gameplay_event_t event;
    event.type = event_type_t::PROVOKE_NPC;
    event.actor_id = npc_id;
    event.target_id = source_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_block_event(entityid attacker_id, entityid target_id) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_BLOCK;
    event.actor_id = attacker_id;
    event.target_id = target_id;
    event.attack_result = attack_result_t::BLOCK;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_damage_event(entityid attacker_id, entityid target_id, int damage) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_DAMAGE;
    event.actor_id = attacker_id;
    event.target_id = target_id;
    event.amount = damage;
    event.attack_result = attack_result_t::HIT;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_death_event(entityid attacker_id, entityid target_id) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_DEATH;
    event.actor_id = attacker_id;
    event.target_id = target_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_award_xp_event(entityid attacker_id, entityid target_id) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_AWARD_XP;
    event.actor_id = attacker_id;
    event.target_id = target_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_drop_inventory_event(entityid target_id) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_DROP_INVENTORY;
    event.actor_id = target_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_player_death_event(entityid target_id) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_PLAYER_DEATH;
    event.actor_id = target_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_weapon_durability_event(entityid attacker_id, entityid target_id) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_WEAPON_DURABILITY;
    event.actor_id = attacker_id;
    event.target_id = target_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_attack_shield_durability_event(entityid defender_id, entityid attacker_id) {
    gameplay_event_t event;
    event.type = event_type_t::ATTACK_SHIELD_DURABILITY;
    event.actor_id = defender_id;
    event.target_id = attacker_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_pull_event(entityid id) {
    gameplay_event_t event;
    event.type = event_type_t::PULL_INTENT;
    event.actor_id = id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_open_door_event(entityid id, vec3 loc) {
    gameplay_event_t event;
    event.type = event_type_t::OPEN_DOOR_INTENT;
    event.actor_id = id;
    event.target_loc = loc;
    return queue_gameplay_event(event);
}

bool gamestate::queue_open_chest_event(entityid id, vec3 loc) {
    gameplay_event_t event;
    event.type = event_type_t::OPEN_CHEST_INTENT;
    event.actor_id = id;
    event.target_loc = loc;
    return queue_gameplay_event(event);
}

bool gamestate::queue_interact_event(entityid id, vec3 loc) {
    gameplay_event_t event;
    event.type = event_type_t::INTERACT_INTENT;
    event.actor_id = id;
    event.target_loc = loc;
    return queue_gameplay_event(event);
}

bool gamestate::queue_pickup_event(entityid id) {
    gameplay_event_t event;
    event.type = event_type_t::PICKUP_INTENT;
    event.actor_id = id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_use_item_event(entityid id, entityid item_id) {
    gameplay_event_t event;
    event.type = event_type_t::USE_ITEM_INTENT;
    event.actor_id = id;
    event.target_id = item_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_equip_item_event(entityid id, entityid item_id) {
    gameplay_event_t event;
    event.type = event_type_t::EQUIP_ITEM_INTENT;
    event.actor_id = id;
    event.target_id = item_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_drop_item_event(entityid id, entityid item_id) {
    gameplay_event_t event;
    event.type = event_type_t::DROP_ITEM_INTENT;
    event.actor_id = id;
    event.item_id = item_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_chest_transfer_event(entityid from_id, entityid to_id, entityid item_id) {
    gameplay_event_t event;
    event.type = event_type_t::CHEST_TRANSFER_INTENT;
    event.actor_id = from_id;
    event.target_id = to_id;
    event.item_id = item_id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_traverse_stairs_event(entityid id) {
    gameplay_event_t event;
    event.type = event_type_t::TRAVERSE_STAIRS_INTENT;
    event.actor_id = id;
    return queue_gameplay_event(event);
}

bool gamestate::queue_pressure_plate_refresh_event(int z) {
    gameplay_event_t event;
    event.type = event_type_t::REFRESH_PRESSURE_PLATES;
    event.floor = z;
    return queue_gameplay_event(event);
}

bool gamestate::queue_pressure_plate_set_door_event(entityid door_id, bool should_open) {
    gameplay_event_t event;
    event.type = event_type_t::PRESSURE_PLATE_SET_DOOR;
    event.actor_id = door_id;
    event.state = should_open;
    return queue_gameplay_event(event);
}

gameplay_event_result_t gamestate::process_gameplay_event(const gameplay_event_t& event) {
    gameplay_event_result_t result;
    result.type = event.type;
    result.actor_id = event.actor_id;
    switch (event.type) {
    case event_type_t::MOVE_INTENT: {
        result.handled = true;
        result.succeeded = try_entity_move(event.actor_id, event.delta);
        if (result.succeeded) {
            const vec3 loc = get_component_or<Position>(event.actor_id, vec3{-1, -1, -1});
            if (vec3_valid(loc)) {
                queue_pressure_plate_refresh_event(loc.z);
            }
        }
        return result;
    }
    case event_type_t::PUSH_INTENT: {
        result.handled = true;
        result.succeeded = try_entity_push(event.actor_id, event.delta);
        if (result.succeeded) {
            const vec3 loc = get_component_or<Position>(event.actor_id, vec3{-1, -1, -1});
            if (vec3_valid(loc)) {
                queue_pressure_plate_refresh_event(loc.z);
            }
        }
        return result;
    }
    case event_type_t::ATTACK_INTENT:
        result.handled = true;
        result.attack_result = resolve_attack_intent(event.actor_id, event.target_loc);
        handle_attack_sfx(event.actor_id, result.attack_result);
        if (!test) {
            set_gamestate_flag_for_attack_animation(
                (get_component<EntityTypeTag>(event.actor_id) ? get_component<EntityTypeTag>(event.actor_id)->type : entitytype_t::NONE));
        }
        result.succeeded = result.attack_result != attack_result_t::NONE;
        return result;
    case event_type_t::PROVOKE_NPC:
        result.handled = true;
        resolve_provoke_npc_event(event.actor_id, event.target_id);
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_BLOCK:
        result.handled = true;
        resolve_attack_block_event(event.actor_id, event.target_id);
        result.attack_result = attack_result_t::BLOCK;
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_DAMAGE:
        result.handled = true;
        resolve_attack_damage_event(event.actor_id, event.target_id, event.amount);
        result.attack_result = attack_result_t::HIT;
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_DEATH:
        result.handled = true;
        resolve_attack_death_event(event.actor_id, event.target_id);
        result.attack_result = attack_result_t::HIT;
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_AWARD_XP:
        result.handled = true;
        resolve_attack_award_xp_event(event.actor_id, event.target_id);
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_DROP_INVENTORY:
        result.handled = true;
        resolve_attack_drop_inventory_event(event.actor_id);
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_PLAYER_DEATH:
        result.handled = true;
        resolve_attack_player_death_event(event.actor_id);
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_WEAPON_DURABILITY:
        result.handled = true;
        resolve_attack_weapon_durability_event(event.actor_id, event.target_id);
        result.succeeded = true;
        return result;
    case event_type_t::ATTACK_SHIELD_DURABILITY:
        result.handled = true;
        resolve_attack_shield_durability_event(event.actor_id, event.target_id);
        result.succeeded = true;
        return result;
    case event_type_t::PULL_INTENT: {
        result.handled = true;
        result.succeeded = try_entity_pull(event.actor_id);
        if (result.succeeded) {
            const vec3 loc = get_component_or<Position>(event.actor_id, vec3{-1, -1, -1});
            if (vec3_valid(loc)) {
                queue_pressure_plate_refresh_event(loc.z);
            }
        }
        return result;
    }
    case event_type_t::OPEN_DOOR_INTENT:
        result.handled = true;
        result.succeeded = try_entity_open_door(event.actor_id, event.target_loc);
        return result;
    case event_type_t::OPEN_CHEST_INTENT:
        result.handled = true;
        result.succeeded = try_entity_open_chest(event.actor_id, event.target_loc);
        return result;
    case event_type_t::INTERACT_INTENT:
        result.handled = true;
        result.succeeded = try_entity_interact(event.actor_id, event.target_loc);
        return result;
    case event_type_t::PICKUP_INTENT:
        result.handled = true;
        result.succeeded = try_entity_pickup(event.actor_id);
        return result;
    case event_type_t::USE_ITEM_INTENT:
        result.handled = true;
        result.succeeded = false;
        if ((get_component<EntityTypeTag>(event.target_id) ? get_component<EntityTypeTag>(event.target_id)->type : entitytype_t::NONE) == entitytype_t::ITEM &&
            get_component_or<ItemSubtype>(event.target_id, itemtype_t::NONE) == itemtype_t::POTION && use_potion(event.actor_id, event.target_id)) {
            if (event.actor_id == hero_id) {
                flag = gamestate_flag_t::PLAYER_ANIM;
                controlmode = controlmode_t::PLAYER;
                ui.display_inventory_menu = false;
            }
            result.succeeded = true;
        }
        return result;
    case event_type_t::EQUIP_ITEM_INTENT:
        result.handled = true;
        result.succeeded = false;
        if (event.actor_id == hero_id &&
            (get_component<EntityTypeTag>(event.target_id) ? get_component<EntityTypeTag>(event.target_id)->type : entitytype_t::NONE) == entitytype_t::ITEM) {
            const itemtype_t item_type = get_component_or<ItemSubtype>(event.target_id, itemtype_t::NONE);
            if (item_type == itemtype_t::WEAPON || item_type == itemtype_t::SHIELD) {
                handle_hero_inventory_equip_item(event.target_id);
                result.succeeded = true;
            }
        }
        return result;
    case event_type_t::DROP_ITEM_INTENT:
        result.handled = true;
        result.succeeded = false;
        if (event.actor_id == hero_id &&
            (get_component<EntityTypeTag>(event.item_id) ? get_component<EntityTypeTag>(event.item_id)->type : entitytype_t::NONE) == entitytype_t::ITEM) {
            result.succeeded = drop_inventory_item(event.actor_id, event.item_id);
        }
        return result;
    case event_type_t::CHEST_TRANSFER_INTENT:
        result.handled = true;
        result.succeeded = transfer_inventory_item(event.actor_id, event.target_id, event.item_id);
        if (result.succeeded) {
            frame_dirty = true;
            audio.queue("sfx/Minifantasy_Dungeon_SFX/02_chest_close_1.wav");
            auto* updated_inventory = get_component<Inventory>(event.actor_id);
            clamp_inventory_selection(updated_inventory ? updated_inventory->value.size() : 0);
        }
        return result;
    case event_type_t::TRAVERSE_STAIRS_INTENT:
        result.handled = true;
        result.succeeded = try_entity_stairs(event.actor_id);
        return result;
    case event_type_t::REFRESH_PRESSURE_PLATES:
        result.handled = true;
        if (event.floor >= 0 && static_cast<size_t>(event.floor) < d.floors.size()) {
            update_pressure_plates_for_floor(event.floor);
            result.succeeded = true;
        }
        return result;
    case event_type_t::PRESSURE_PLATE_SET_DOOR:
        result.handled = true;
        resolve_pressure_plate_set_door_event(event.actor_id, event.state);
        result.succeeded = true;
        return result;
    case event_type_t::NONE:
    case event_type_t::COUNT:
    default: return result;
    }
}

gameplay_event_result_t gamestate::process_gameplay_events() {
    gameplay_event_result_t first_result;
    if (processing_actions) {
        return first_result;
    }

    processing_actions = true;
    // FIFO iteration over a growable vector is the ordering contract for the
    // current gameplay-event migration. Handlers may append follow-up events
    // while processing earlier ones; those appended events must execute later
    // in the same turn slice, in append order.
    for (size_t i = 0; i < gameplay_events.size(); i++) {
        const gameplay_event_result_t result = process_gameplay_event(gameplay_events[i]);
        if (first_result.type == event_type_t::NONE && result.type != event_type_t::NONE) {
            first_result = result;
        }
    }
    gameplay_events.clear();
    processing_actions = false;
    return first_result;
}

bool gamestate::run_move_action(entityid id, vec3 v) {
    clear_gameplay_events();
    if (!queue_move_event(id, v)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_push_action(entityid id, vec3 v) {
    clear_gameplay_events();
    if (!queue_push_event(id, v)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

attack_result_t gamestate::run_attack_action(entityid id, vec3 loc) {
    clear_gameplay_events();
    if (!queue_attack_event(id, loc)) {
        return attack_result_t::NONE;
    }
    return process_gameplay_events().attack_result;
}

bool gamestate::run_pull_action(entityid id) {
    clear_gameplay_events();
    if (!queue_pull_event(id)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_open_door_action(entityid id, vec3 loc) {
    clear_gameplay_events();
    if (!queue_open_door_event(id, loc)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_open_chest_action(entityid id, vec3 loc) {
    clear_gameplay_events();
    if (!queue_open_chest_event(id, loc)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_interact_action(entityid id, vec3 loc) {
    clear_gameplay_events();
    if (!queue_interact_event(id, loc)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_pickup_action(entityid id) {
    clear_gameplay_events();
    if (!queue_pickup_event(id)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_use_item_action(entityid id, entityid item_id) {
    clear_gameplay_events();
    if (!queue_use_item_event(id, item_id)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_equip_item_action(entityid id, entityid item_id) {
    clear_gameplay_events();
    if (!queue_equip_item_event(id, item_id)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_drop_item_action(entityid id, entityid item_id) {
    clear_gameplay_events();
    if (!queue_drop_item_event(id, item_id)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_chest_transfer_action(entityid from_id, entityid to_id, entityid item_id) {
    clear_gameplay_events();
    if (!queue_chest_transfer_event(from_id, to_id, item_id)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::run_traverse_stairs_action(entityid id) {
    clear_gameplay_events();
    if (!queue_traverse_stairs_event(id)) {
        return false;
    }
    return process_gameplay_events().succeeded;
}

bool gamestate::try_entity_move(entityid id, vec3 v) {
    massert(id != ENTITYID_INVALID, "Entity ID is invalid!");
    minfo2("entity %d is trying to move: (%d,%d,%d)", id, v.x, v.y, v.z);
    massert(has_component<Position>(id), "id %d has no location", id);

    vec3 loc = get_component_or<Position>(id, vec3{-1, -1, -1});
    massert(!vec3_invalid(loc), "id %d location invalid", id);
    vec3 aloc = {loc.x + v.x, loc.y + v.y, loc.z};

    minfo2("entity %d is trying to move to (%d,%d,%d)", id, aloc.x, aloc.y, aloc.z);

    shared_ptr<dungeon_floor> df = d.get_floor(loc.z);
    if (aloc.x < 0 || aloc.x >= df->get_width() || aloc.y < 0 || aloc.y >= df->get_height()) {
        merror2("destination is invalid: (%d, %d, %d)", aloc.x, aloc.y, aloc.z);
        return false;
    }

    tile_t& tile = df->tile_at(aloc);
    if (!tile_is_walkable(tile.get_type())) {
        if (!(god_mode && id == hero_id)) {
            merror2("tile is not walkable");
            return false;
        }
    }
    entityid box_id = tile_has_box(aloc.x, aloc.y, aloc.z);
    if (box_id != ENTITYID_INVALID) {
        merror2("box present, trying to push");
        return handle_box_push(box_id, v);
    }
    entityid pushable_id = tile_has_pushable(aloc.x, aloc.y, aloc.z);
    if (pushable_id != ENTITYID_INVALID) {
        merror2("pushable present, trying to push");
        return handle_box_push(pushable_id, v);
    }
    bool has_solid = tile_has_solid(aloc.x, aloc.y, aloc.z);
    if (has_solid) {
        merror2("solid present, cannot move");
        return false;
    }
    else if (tile.get_cached_live_npc() != INVALID) {
        merror2("live npcs present, cannot move");
        return false;
    }

    entityid door_id = tile_has_door(aloc);
    if (door_id != ENTITYID_INVALID) {
        massert(has_component<DoorOpenFlag>(door_id), "door_id %d doesnt have a door_open component", door_id);
        if (!get_component_or<DoorOpenFlag>(door_id, false)) {
            merror2("door is closed");
            return false;
        }
    }

    if (!df->df_remove_at(id, loc)) {
        merror2("Failed to remove %d from (%d, %d)", id, loc.x, loc.y);
        return false;
    }

    auto type = (get_component<EntityTypeTag>(id) ? get_component<EntityTypeTag>(id)->type : entitytype_t::NONE);
    bool is_dead_npc = type == entitytype_t::NPC && get_component_or<DeadFlag>(id, false);
    if (is_dead_npc) {
        tile_t& dst_tile = df->tile_at(aloc);
        dst_tile.add_dead_npc(id);
    }
    else if (df->df_add_at(id, type, aloc) == ENTITYID_INVALID) {
        merror2("Failed to add %d to (%d, %d)", id, aloc.x, aloc.y);
        return false;
    }

    sync_registry_grid_position(id, aloc);
    float mx = v.x * DEFAULT_TILE_SIZE;
    float my = v.y * DEFAULT_TILE_SIZE;
    if (auto* pos = get_component<Position>(id)) {
        pos->value = aloc;
    }
    if (auto* sm = get_component<SpriteMoveState>(id)) {
        sm->value = Rectangle{mx, my, 0, 0};
    }
    if (auto* steps = get_component<StepsTaken>(id)) {
        steps->value = get_component_or<StepsTaken>(id, 0u) + 1;
    }
    if (auto* upd = get_component<NeedsUpdate>(id)) {
        upd->value = true;
    }
    if (auto* facing = get_component<Facing>(id)) {
        facing->value = get_dir_from_loc(v);
    }
    if (check_hearing(hero_id, aloc)) {
        audio.queue("sfx/Minifantasy_Dungeon_SFX/16_human_walk_stone_1.wav");
    }
    if (!processing_actions) {
        update_pressure_plates_for_floor(loc.z);
    }
    msuccess2("npc %d moved to (%d,%d,%d)", id, aloc.x, aloc.y, aloc.z);
    return true;
}

bool gamestate::handle_move_up(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_UP)) {
        if (is_dead) {
            return messages.add("You cannot move while dead");
        }
        run_move_action(hero_id, vec3{0, -1, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

bool gamestate::handle_move_down(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_DOWN)) {
        if (is_dead) {
            return messages.add("You cannot move while dead");
        }
        run_move_action(hero_id, vec3{0, 1, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

bool gamestate::handle_move_left(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_LEFT)) {
        if (is_dead) {
            return messages.add("You cannot move while dead");
        }
        run_move_action(hero_id, vec3{-1, 0, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

bool gamestate::handle_move_right(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_RIGHT)) {
        if (is_dead) {
            return messages.add("You cannot move while dead");
        }
        run_move_action(hero_id, vec3{1, 0, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

bool gamestate::handle_move_up_left(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_UP_LEFT)) {
        if (is_dead) {
            return messages.add("You cannot move while dead");
        }
        run_move_action(hero_id, vec3{-1, -1, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

bool gamestate::handle_move_up_right(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_UP_RIGHT)) {
        if (is_dead) {
            messages.add("You cannot move while dead");
            return true;
        }
        run_move_action(hero_id, vec3{1, -1, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

bool gamestate::handle_move_down_left(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_DOWN_LEFT)) {
        if (is_dead) {
            return messages.add("You cannot move while dead");
        }
        run_move_action(hero_id, vec3{-1, 1, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

bool gamestate::handle_move_down_right(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::MOVE_DOWN_RIGHT)) {
        if (is_dead) {
            return messages.add("You cannot move while dead");
        }
        run_move_action(hero_id, vec3{1, 1, 0});
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}

vec3 gamestate::get_loc_facing_player() {
    const Position* maybe_loc = get_component<Position>(hero_id);
    if (maybe_loc != nullptr) {
        vec3 loc = maybe_loc->value;
        direction_t dir = get_component_or<Facing>(hero_id, direction_t::NONE);
        if (dir == direction_t::UP) {
            loc.y -= 1;
        }
        else if (dir == direction_t::DOWN) {
            loc.y += 1;
        }
        else if (dir == direction_t::LEFT) {
            loc.x -= 1;
        }
        else if (dir == direction_t::RIGHT) {
            loc.x += 1;
        }
        else if (dir == direction_t::UP_LEFT) {
            loc.x -= 1;
            loc.y -= 1;
        }
        else if (dir == direction_t::UP_RIGHT) {
            loc.x += 1;
            loc.y -= 1;
        }
        else if (dir == direction_t::DOWN_LEFT) {
            loc.x -= 1;
            loc.y += 1;
        }
        else if (dir == direction_t::DOWN_RIGHT) {
            loc.x += 1;
            loc.y += 1;
        }
        return loc;
    }
    return vec3{-1, -1, -1};
}

entityid gamestate::tile_get_item(shared_ptr<tile_t> t) {
    return t->get_cached_item();
}

bool gamestate::try_entity_pull(entityid id) {
    minfo("try_entity_pull(%d)", id);
    massert(id != ENTITYID_INVALID, "Entity is NULL!");
    vec3 loc = get_component_or<Position>(id, vec3{-1, -1, -1});
    massert(!vec3_invalid(loc), "loc is invalid");
    auto df = d.get_floor(loc.z);
    direction_t facing_d = get_component_or<Facing>(id, direction_t::NONE);
    massert(facing_d != direction_t::NONE, "direction d is none");
    direction_t d = get_opposite_dir(facing_d);
    vec3 v = get_loc_from_dir(d);
    vec3 aloc = {loc.x + v.x, loc.y + v.y, loc.z};
    vec3 fv = get_loc_from_dir(facing_d);
    vec3 bloc = {loc.x + fv.x, loc.y + fv.y, loc.z};

    if (aloc.x < 0 || aloc.x >= df->get_width() || aloc.y < 0 || aloc.y >= df->get_height()) {
        merror("destination is invalid: (%d, %d, %d)", aloc.x, aloc.y, aloc.z);
        return false;
    }

    if (bloc.x < 0 || bloc.x >= df->get_width() || bloc.y < 0 || bloc.y >= df->get_height()) {
        merror("pull source is invalid: (%d, %d, %d)", bloc.x, bloc.y, bloc.z);
        return false;
    }

    tile_t& tile_dest = df->tile_at(aloc);

    if (!tile_is_walkable(tile_dest.get_type())) {
        merror("tile is not walkable");
        return false;
    }

    entityid box_id = tile_has_box(aloc.x, aloc.y, aloc.z);
    if (box_id != ENTITYID_INVALID) {
        merror("box present, can't push and pull simultaneously");
        return false;
    }

    bool has_solid = tile_has_solid(aloc.x, aloc.y, aloc.z);
    if (has_solid) {
        merror("solid present, cannot move");
        return false;
    }

    if (tile_dest.get_cached_live_npc() != INVALID) {
        merror("live npcs present, cannot move");
        return false;
    }

    //if (tile_at_cur_floor(aloc).get_cached_player_present()) {
    //    merror("player present, cannot move");
    //    return false;
    //}

    entityid door_id = tile_has_door(aloc);
    if (door_id != ENTITYID_INVALID) {
        massert(has_component<DoorOpenFlag>(door_id), "door_id %d doesnt have a door_open component", door_id);
        if (!get_component_or<DoorOpenFlag>(door_id, false)) {
            merror("door is closed");
            return false;
        }
    }

    entityid box_id2 = tile_has_pullable(bloc.x, bloc.y, bloc.z);
    if (box_id2 == ENTITYID_INVALID) {
        return false;
    }

    if (!df->df_remove_at(id, loc)) {
        merror("Failed to remove %d from (%d, %d)", id, loc.x, loc.y);
        return false;
    }

    auto type = (get_component<EntityTypeTag>(id) ? get_component<EntityTypeTag>(id)->type : entitytype_t::NONE);
    bool is_dead_npc = type == entitytype_t::NPC && get_component_or<DeadFlag>(id, false);
    if (is_dead_npc) {
        tile_t& dst_tile = df->tile_at(aloc);
        dst_tile.add_dead_npc(id);
    }

    if (df->df_add_at(id, type, aloc) == ENTITYID_INVALID) {
        merror("Failed to add %d to (%d, %d)", id, aloc.x, aloc.y);
        return false;
    }

    sync_registry_grid_position(id, aloc);
    float mx = v.x * DEFAULT_TILE_SIZE;
    float my = v.y * DEFAULT_TILE_SIZE;
    if (auto* pos = get_component<Position>(id)) {
        pos->value = aloc;
    }
    if (auto* sm = get_component<SpriteMoveState>(id)) {
        sm->value = Rectangle{mx, my, 0, 0};
    }
    if (auto* steps = get_component<StepsTaken>(id)) {
        steps->value = get_component_or<StepsTaken>(id, 0u) + 1;
    }
    if (auto* upd = get_component<NeedsUpdate>(id)) {
        upd->value = true;
    }
    if (check_hearing(hero_id, aloc)) {
        audio.queue("sfx/Minifantasy_Dungeon_SFX/16_human_walk_stone_1.wav");
    }

    msuccess("npc %d moved to (%d,%d,%d)", id, aloc.x, aloc.y, aloc.z);
    try_entity_move(box_id2, v);
    if (!processing_actions) {
        update_pressure_plates_for_floor(loc.z);
    }
    msuccess("try_entity_pull(%d)", id);
    return true;
}

bool gamestate::try_entity_pickup(entityid id) {
    massert(id != ENTITYID_INVALID, "Entity is NULL!");
    const Position* maybe_loc = get_component<Position>(id);
    if (maybe_loc == nullptr) {
        merror("id %d has no location", id);
        return false;
    }
    vec3 loc = maybe_loc->value;
    shared_ptr<dungeon_floor> df = d.get_floor(loc.z);
    tile_t& tile = df->tile_at(loc);
    bool item_picked_up = false;
    entityid item_id = tile.get_cached_item();
    if (item_id != ENTITYID_INVALID && add_to_inventory(id, item_id)) {
        tile.tile_remove(item_id);
        audio.queue("sfx/Minifantasy_Dungeon_SFX/02_chest_close_1.wav");
        item_picked_up = true;
        string item_name = get_component_or<EntityName>(item_id, "no-name-item");
        messages.add_history("You picked up %s", item_name.c_str());
    }
    else if (item_id == ENTITYID_INVALID) {
        mwarning("No item cached");
    }
    entitytype_t t = (get_component<EntityTypeTag>(id) ? get_component<EntityTypeTag>(id)->type : entitytype_t::NONE);
    if (t == entitytype_t::PLAYER) {
        flag = gamestate_flag_t::PLAYER_ANIM;
    }
    return item_picked_up;
}

bool gamestate::handle_pickup_item(inputstate& is, bool is_dead) {
    if (!is_action_pressed(is, gameplay_input_action_t::PICKUP)) {
        return false;
    }
    if (is_dead) {
        return messages.add("You cannot pick up items while dead");
    }
    run_pickup_action(hero_id);
    flag = gamestate_flag_t::PLAYER_ANIM;
    return true;
}

bool gamestate::try_entity_stairs(entityid id) {
    massert(id != ENTITYID_INVALID, "Entity ID is invalid!");
    const Position* maybe_loc = get_component<Position>(id);
    massert(maybe_loc != nullptr, "id %d has no location", id);
    vec3 loc = maybe_loc->value;
    int current_floor = d.current_floor;
    shared_ptr<dungeon_floor> df = d.floors[current_floor];
    tile_t& t = df->tile_at(loc);
    if (t.get_type() == tiletype_t::UPSTAIRS) {
        if (current_floor == 0) {
            messages.add("You are already on the top floor!");
        }
        else {
            df->df_remove_at(hero_id, loc);
            d.current_floor--;
            int new_floor = d.current_floor;
            shared_ptr<dungeon_floor> df2 = d.floors[new_floor];
            vec3 uloc = df2->get_downstairs_loc();
            df2->df_add_at(hero_id, entitytype_t::PLAYER, uloc);
            if (processing_actions) {
                queue_pressure_plate_refresh_event(current_floor);
                queue_pressure_plate_refresh_event(new_floor);
            }
            else {
                refresh_pressure_plates();
            }
            flag = gamestate_flag_t::PLAYER_ANIM;
            audio.queue("sfx/Minifantasy_Dungeon_SFX/16_human_walk_stone_1.wav");
            return true;
        }
    }
    else if (t.get_type() == tiletype_t::DOWNSTAIRS) {
        if (static_cast<size_t>(current_floor) < d.floors.size() - 1) {
            df->df_remove_at(hero_id, loc);
            d.current_floor++;
            int new_floor = d.current_floor;
            shared_ptr<dungeon_floor> df2 = d.floors[new_floor];
            vec3 uloc = df2->get_upstairs_loc();
            df2->df_add_at(hero_id, entitytype_t::PLAYER, uloc);
            if (processing_actions) {
                queue_pressure_plate_refresh_event(current_floor);
                queue_pressure_plate_refresh_event(new_floor);
            }
            else {
                refresh_pressure_plates();
            }
            flag = gamestate_flag_t::PLAYER_ANIM;
            audio.queue("sfx/Minifantasy_Dungeon_SFX/16_human_walk_stone_1.wav");
            return true;
        }
        else {
            messages.add("You can't go downstairs anymore!");
        }
    }
    return false;
}

bool gamestate::handle_traverse_stairs(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::STAIRS)) {
        if (is_dead) {
            return messages.add("You cannot traverse stairs while dead");
        }
        run_traverse_stairs_action(hero_id);
        return true;
    }
    return false;
}

bool gamestate::try_entity_open_door(entityid id, vec3 loc) {
    massert(id != ENTITYID_INVALID, "id is invalid");
    const entityid door_id = tile_has_door(loc);
    if (door_id == ENTITYID_INVALID) {
        return false;
    }
    massert(tile_has_door(loc) == door_id, "door cache mismatch at (%d, %d, %d)", loc.x, loc.y, loc.z);
    if (door_is_pressure_plate_controlled(door_id)) {
        return messages.add("You cannot open this door with your hands");
    }
    const DoorOpenFlag* maybe_is_open = get_component<DoorOpenFlag>(door_id);
    massert(maybe_is_open != nullptr, "door %d has no `is_open` component", door_id);
    sync_registry_open_state(door_id, !maybe_is_open->value);
    audio.queue("sfx/Minifantasy_Dungeon_SFX/01_chest_open_1.wav");
    return true;
}

bool gamestate::try_entity_open_chest(entityid id, vec3 loc) {
    massert(id != ENTITYID_INVALID, "id is invalid");
    const entityid chest_id = tile_has_chest(loc.x, loc.y, loc.z);
    if (chest_id == ENTITYID_INVALID) {
        return false;
    }
    const DoorOpenFlag* maybe_is_open = get_component<DoorOpenFlag>(chest_id);
    massert(maybe_is_open != nullptr, "chest %d has no open state component", chest_id);
    if (maybe_is_open->value) {
        close_chest_menu();
        return true;
    }
    return open_chest_menu(chest_id);
}

bool gamestate::try_entity_interact(entityid id, vec3 loc) {
    massert(id != ENTITYID_INVALID, "id is invalid");
    shared_ptr<dungeon_floor> df = d.get_floor(loc.z);
    tile_t& tile = df->tile_at(loc);

    const entityid npc_id = tile.get_cached_live_npc();
    if (npc_id != ENTITYID_INVALID && npc_id != hero_id) {
        const string speaker_name = get_component_or<EntityName>(npc_id, "Someone");
        const string text = get_component_or<DialogueLine>(npc_id, "They have nothing to say.");
        open_interaction_modal(npc_id, speaker_name, text);
        return true;
    }

    const entityid prop_id = tile.get_cached_prop();
    if (prop_id != ENTITYID_INVALID) {
        const string prop_name = get_component_or<EntityName>(prop_id, "Prop");
        const string text = get_component_or<EntityDescription>(prop_id, "There is nothing notable about it.");
        open_interaction_modal(prop_id, prop_name, text);
        return true;
    }

    const entityid box_id = tile.get_cached_box();
    if (box_id != ENTITYID_INVALID) {
        const string box_name = get_component_or<EntityName>(box_id, "Box");
        const string text = get_component_or<EntityDescription>(box_id, "A plain wooden box.");
        open_interaction_modal(box_id, box_name, text);
        return true;
    }

    const entityid chest_id = tile.get_cached_chest();
    if (chest_id != ENTITYID_INVALID) {
        const string chest_name = get_component_or<EntityName>(chest_id, "Treasure chest");
        const bool is_open = get_component_or<DoorOpenFlag>(chest_id, false);
        const string base_text =
            get_component_or<EntityDescription>(chest_id, "A stout treasure chest reinforced with iron bands and built to survive rough handling.");
        const string text = interaction_chest_text(base_text, is_open);
        open_interaction_modal(chest_id, chest_name, text);
        return true;
    }

    const entityid door_id = tile.get_cached_door();
    if (door_id != ENTITYID_INVALID) {
        const string door_name = get_component_or<EntityName>(door_id, "Door");
        const bool is_open = get_component_or<DoorOpenFlag>(door_id, false);
        const string base_text = get_component_or<EntityDescription>(door_id, "A heavy wooden door bound with iron straps and swollen from the dungeon damp.");
        open_interaction_modal(door_id, door_name, interaction_door_text(base_text, is_open));
        return true;
    }

    return false;
}

bool gamestate::handle_interact(inputstate& is, bool is_dead) {
    if (!is_action_pressed(is, gameplay_input_action_t::INTERACT)) {
        return false;
    }
    if (is_dead) {
        return messages.add("You cannot interact while dead");
    }
    vec3 loc = get_loc_facing_player();
    if (!run_interact_action(hero_id, loc)) {
        return messages.add("There is nothing to interact with");
    }
    flag = gamestate_flag_t::PLAYER_ANIM;
    return true;
}

bool gamestate::handle_open_door(inputstate& is, bool is_dead) {
    if (is_action_pressed(is, gameplay_input_action_t::OPEN)) {
        if (is_dead) {
            return messages.add("You cannot open doors while dead");
        }
        vec3 loc = get_loc_facing_player();
        if (!run_open_chest_action(hero_id, loc)) {
            run_open_door_action(hero_id, loc);
        }
        flag = gamestate_flag_t::PLAYER_ANIM;
        return true;
    }
    return false;
}
