/** @file world_renderer.cpp
 *  @brief WorldRenderer implementation — dungeon floor tiles, pressure plates, entities, damage numbers.
 */
...
bool WorldRenderer::draw_floor_tile(
    gamestate& g,
    rpg::Renderer& renderer,
    int x,
    int y,
    int z,
    [[maybe_unused]] int light_dist,
    [[maybe_unused]] vec3 hero_loc,
    [[maybe_unused]] int distance) {
...
void WorldRenderer::draw_floor_pressure_plates(gamestate& g, rpg::Renderer& renderer, int light_rad) {
...
void WorldRenderer::draw_floor_entitytype(
    gamestate& g,
    rpg::Renderer& renderer,
    entitytype_t type_0,
    [[maybe_unused]] int vision_dist,
    [[maybe_unused]] int light_rad,
...
bool WorldRenderer::draw_floor(gamestate& g, rpg::Renderer& renderer, int vision_dist, int light_rad) {
    ...
    for (int y = 0; y < df->get_height(); y++) {
        for (int x = 0; x < df->get_width(); x++) {
            ...
            draw_floor_tile(g, renderer, x, y, z, light_rad, hero_loc, distance);
        }
    }
    draw_floor_pressure_plates(g, renderer, light_rad);
    ...
    draw_player_target_box(g, renderer);
    draw_floor_entitytype(g, renderer, entitytype_t::NPC, vision_dist, light_rad, dead_check);
    draw_floor_entitytype(g, renderer, entitytype_t::NPC, vision_dist, light_rad, alive_check);
    draw_floor_entitytype(g, renderer, entitytype_t::PLAYER, vision_dist, light_rad, mydefault);
    return true;
}
...
void WorldRenderer::draw_damage_numbers(gamestate& g) {
